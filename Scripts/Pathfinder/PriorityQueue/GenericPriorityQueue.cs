...
public class PriorityQueue<T> where T : IComparable<T>
{
    ...
    public T Dequeue()
    {
        ...
        while (true)
        {
            int childIndex = parentIdenx * 2 + 1;
            if (childIndex > lastIndex) break;
            int rc = childIndex + 1;
            if (rc <= lastIndex && data[rc].CompareTo(data[childIndex]) < 0) childIndex = rc;
            if (data[parentIdenx].CompareTo(data[childIndex]) <= 0) break;
            T tmp = data[parentIdenx];
            data[parentIdenx] = data[childIndex];
            data[childIndex] = tmp;
            parentIdenx = childIndex;
        }
